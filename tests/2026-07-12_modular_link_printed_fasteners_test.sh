...
if [[ ! -f "$source_model" ]]; then
    echo "FAIL: missing printed fastener source: $source_model" >&2
    exit 1
fi

if [[ ! -x "$export_script" ]]; then
    echo "FAIL: missing executable printed-fastener export script: $export_script" >&2
    exit 1
fi

if [[ ! -x "$auditor" && ! -f "$auditor" ]]; then
...
render_part() {
    ...
    grep -q 'FASTENER_PARAMETERS' "$log_file"
    grep -q 'bolt_outer_d=4.2' "$log_file"
    grep -q 'thread_pitch=1' "$log_file"
    grep -q 'nut_thread_cut_d=4.65' "$log_file"
    grep -q 'link_bolt_clearance_d=4.6' "$log_file"
...
artifact_path() {
    case "$1" in
        hex_bolt)
            echo "$artifact_root/${date_prefix}_modular_link_hex_bolt.stl"
            ;;
        hex_nut)
            echo "$artifact_root/${date_prefix}_modular_link_hex_nut.stl"
            ;;
        wing_nut)
            echo "$artifact_root/${date_prefix}_modular_link_wing_nut.stl"
            ;;
        *)
            echo "FAIL: unknown part name: $1" >&2
            exit 1
            ;;
    esac
}
...
render_part hex_bolt 9.2377 8 22.6 0.08
render_part hex_nut 9.2377 8 4.5 0.08
render_part wing_nut 28 10 4.8 0.08

for part_name in hex_bolt hex_nut wing_nut; do
    assert_artifact "$part_name"
done

artifact_count="$(find "$artifact_root" -maxdepth 1 -type f -name "${date_prefix}_modular_link_*.stl" | wc -l | tr -d ' ')"
if [[ "$artifact_count" != "3" ]]; then
    echo "FAIL: expected 3 dated printed-fastener STL files, found $artifact_count" >&2
    exit 1
fi
...
echo "PASS: modular-link printed fasteners are valid"
