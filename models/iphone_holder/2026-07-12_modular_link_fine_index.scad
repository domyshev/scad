// Fine-index modular iPhone-stand link.
//
// This variant keeps the 2026-07-11 link envelope but doubles the working
// serration count from 24 to 48, giving 7.5-degree indexed joint positions.

$fn = 96;

scene = "presentation"; // single_link, fit_pair_print, exploded_joint, assembled_joint, joint_overlap, presentation
show_hardware = true;
joint_angle = 45;
...
echo(str("FINE_INDEX_LINK tooth_count=", tooth_count, " step_angle=", tooth_pitch_angle));

function modular_link_fine_index_step_angle() = tooth_pitch_angle;
function modular_link_fine_index_tooth_count() = tooth_count;
...
module presentation() {
    translate([-70, 45, 0]) {
        ...
        translate([0, -27, 0])
            drawing_label("1  FINE INDEX LINK");
    ...
    translate([55, 45, 0]) {
        ...
        translate([-15, -34, 0])
            drawing_label("2  EXPLODED JOINT");

        translate([-15, -42, 0])
            drawing_label(
                str("INDEX ", tooth_pitch_angle, " DEG | M4"),
                4
            );
    }
