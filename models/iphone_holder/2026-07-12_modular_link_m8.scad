// M8 fine-index modular iPhone-stand link.
//
// This single-link variant keeps the 7.5-degree index, but enlarges the joint
// pad and serration ring so an M8 clearance hole does not leave a thin web.
...
echo(
    str(
        ...
        " step_angle=", tooth_pitch_angle,
        " joint_d=", joint_d,
        " bolt_clearance_d=", bolt_clearance_d,
        " tooth_inner_d=", tooth_inner_d,
        " tooth_outer_d=", tooth_outer_d
    )
);

function modular_link_m8_step_angle() = tooth_pitch_angle;
function modular_link_m8_tooth_count() = tooth_count;
function modular_link_m8_joint_d() = joint_d;
function modular_link_m8_bolt_clearance_d() = bolt_clearance_d;
function modular_link_m8_tooth_inner_d() = tooth_inner_d;
function modular_link_m8_tooth_outer_d() = tooth_outer_d;
...
module m8_bolt() {
    bolt_bottom = -m8_washer_h - m8_bolt_head_h;
    bolt_top = assembly_stack_h + m8_washer_h + m8_nut_h;

    union() {
        translate([0, 0, bolt_bottom])
            cylinder(d = m8_bolt_head_d, h = m8_bolt_head_h);

        translate([0, 0, bolt_bottom])
            cylinder(d = m8_bolt_d, h = bolt_top - bolt_bottom);
    }
}

module m8_washer_at(z_position) {
    translate([0, 0, z_position])
        difference() {
            cylinder(d = m8_washer_d, h = m8_washer_h);
            translate([0, 0, -0.1])
                cylinder(
                    d = bolt_clearance_d,
                    h = m8_washer_h + 0.2
...
module m8_nut_at(z_position) {
    nut_vertex_d = m8_nut_flat / cos(30);

    translate([0, 0, z_position])
        difference() {
            rotate([0, 0, 30])
                cylinder(d = nut_vertex_d, h = m8_nut_h, $fn = 6);
            translate([0, 0, -0.1])
                cylinder(d = m8_bolt_d, h = m8_nut_h + 0.2);
        }
}

module m8_hardware(top_offset = 0) {
    m8_bolt();
    m8_washer_at(-m8_washer_h);
    m8_washer_at(assembly_stack_h + top_offset);
    m8_nut_at(assembly_stack_h + top_offset + m8_washer_h);
}
...
module exploded_joint() {
    ...
    if (show_hardware)
        color(hardware_color)
            m8_hardware(top_offset = exploded_gap);
}
...
module presentation() {
    translate([-70, 45, 0]) {
        ...
        translate([0, -31, 0])
            drawing_label("1  M8 FINE INDEX LINK");
        ...
        translate([0, -46, 0])
            drawing_label(
                str("48 TEETH | STEP ", tooth_pitch_angle, " DEG"),
                3.8
            );
    }
    ...
    translate([55, 45, 0]) {
        ...
        translate([-15, -38, 0])
            drawing_label("2  M8 EXPLODED JOINT");

        translate([-15, -46, 0])
            drawing_label(
                str("RING ", tooth_inner_d, "-", tooth_outer_d, " MM"),
                4
            );
    }
    ...
    translate([0, -60, 0]) {
        ...
        translate([-5, -38, 0])
            drawing_label(str("3  ASSEMBLED ", joint_angle, " DEG"));
