...
// Cylindrical cut along local radial axis (⊥ to pocket back wall in XZ), for a perpendicular peg hole.
tab_perpendicular_peg_d = 4;
tab_perpendicular_peg_z = 2 + 1.6;
// Length must span the wall through the pocket (boolean is symmetric about pocket center in Y).
...
// Radial cylinder at pocket mid-depth; axis ⊥ XZ back wall of tab_recess_pocket (same line as slot depth).
module tab_perpendicular_peg_cutout(y_sign) {
    y_center = y_sign * (base_outer_r - tab_recess_depth / 2);
    translate([0, y_center+10, tab_perpendicular_peg_z])
        rotate([90, 0, 0])
...
module tab_recess_cutout() {
    ...
    tab_perpendicular_peg_cutout(-1);
}
